        ...
        data.read('checks', id, (error, checksData) => {
            if (!error && checksData) {
                ...
                tokenHandler._token.verify(token, checkObj.userPhone, (tokenIsValid) => {
                    if (tokenIsValid) {
                        // delete check data 
                        data.delete('checks', id, (error1) => {
                            if (!error1) {
                                // user particular check data delete
                                data.read('users', checkObj.userPhone, (error2, userData) => {
                                    ...
                                    if (!error2 && userData) {
                                        ...
                                        // remove the deleted checks id from the user list of checks 
                                        let checkPosition = userChecks.indexOf(id);
                                        if (checkPosition > -1) {
                                            userChecks.splice(checkPosition, 1);
                                            // re save the user data 
                                            userObject.checks = userChecks;
                                            data.update('users', userObject.phone, userObject, (error3) => {
                                                if (!error3) {
                                                    callback(200);
                                                } else {
                                                    callback(500, {
                                            ...
                                            callback(500, {
                                                error: `Check id not found in the user.!`
                                            })
                                        }
                                    } else {
                                        callback(500, {
